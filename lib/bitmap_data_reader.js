...
var meta = function (bitmapObject){
  ...
  fs.readFile(bitmapObject.path, function(err, bitmap) {
    ...
    ee.emit('transform', bitmapObject); //put in index?
  });
  //returns are primarily for testing
  return bitmapObject.buffer ? bitmapObject : setTimeout(function(){ return bitmapObject }, 10);
}
...
var transformObject = function(bitmapObject){
  ...
  return bitmapObject.colors ? bitmapObject : setTimeout(function(){ return bitmapObject }, 10); 
}
var writeToFile = function(bitmapObject){
  fs.writeFile('./write-file.bmp', bitmapObject.buffer);
  console.log('file has been writtin');
}
